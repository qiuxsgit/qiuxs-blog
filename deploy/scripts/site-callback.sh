#!/usr/bin/env bash
set -Eeuo pipefail
[[ $# -ge 6 && $# -le 7 ]] || { echo 'usage: site-callback.sh URL SECRET RELEASE_ID PUBLISH_JOB_ID STATUS BUILD_NUMBER [FAILED_STAGE]' >&2; exit 1; }
...
[[ "$status" =~ ^(queued|building|deploying|success|failed)$ ]] || { echo 'invalid status' >&2; exit 1; }
