...
func TestDevelopSQLReleaseDomainsAndForeignKeysAreExactAndCaseSensitive(t *testing.T) {
	...
	require.NotContains(t, normalized, "FOREIGN KEY")
	require.NotContains(t, normalized, "REFERENCES")
...
func TestDevelopSQLDefinesStageTwoRelationsWithoutForeignKeys(t *testing.T) {
	normalized := readDevelopSQL(t)
	require.NotContains(t, normalized, "FOREIGN KEY")
	require.NotContains(t, normalized, "REFERENCES")
